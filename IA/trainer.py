...
first_size = 49
possible_actions = 7
first_hidden_layers_size_terrestre = 80
snd_hidden_layers_size_terrestre = 80
...
optimizer_terre = tf.train.AdagradOptimizer(learning_rate).minimize(cost_terre,var_list=[first_weights_terre,snd_weights_terre,third_weights_terre,last_weights_terre,first_biases_terre,snd_biases_terre,third_biases_terre,last_biases_terre],global_step = global_step)
...
first_hidden_layers_size_flight = 80
snd_hidden_layers_size_flight = 80  
...
third_weights_flight = (tf.Variable(tf.truncated_normal([first_hidden_layers_size_terrestre,snd_hidden_layers_size_terrestre], stddev=0.1)))
third_biases_flight = (tf.Variable(tf.truncated_normal([snd_hidden_layers_size_terrestre], stddev=0.1)))
...
optimizer_flight = tf.train.AdagradOptimizer(learning_rate).minimize(cost_flight,var_list=[first_biases_flight,snd_biases_flight,third_biases_flight,last_biases_flight,first_weights_flight,snd_weights_flight,third_weights_flight,last_weights_flight],global_step = global_step)
...
first_hidden_layers_size_boat = 80
snd_hidden_layers_size_boat = 80  
...
third_weights_boat = (tf.Variable(tf.truncated_normal([first_hidden_layers_size_terrestre,snd_hidden_layers_size_terrestre], stddev=0.1)))
third_biases_boat = (tf.Variable(tf.truncated_normal([snd_hidden_layers_size_terrestre], stddev=0.1)))
...
optimizer_boat = tf.train.AdagradOptimizer(learning_rate).minimize(cost_boat,var_list=[ first_biases_boat,  snd_biases_boat,third_biases_boat, last_biases_boat,first_weights_boat,snd_weights_boat,third_weights_boat,last_weights_boat],global_step = global_step)
...
#CITY LAYER
first_hidden_layers_size_city = 80 #TODO : A remplacer par le bon nombre de pocibilité
snd_hidden_layers_size_city = 80
...
optimizer_city= tf.train.AdagradOptimizer(learning_rate).minimize(cost_city,var_list=[first_biases_city,  snd_biases_city,  third_biases_city ,last_biases_city,first_weights_city,snd_weights_city,third_weights_city,last_weights_city],global_step = global_step)
...
def cls_accuracy(correct):
    ...
    for i in range (len(correct)) :
        correct_tab = correct[i]
        for j in range(len(correct_tab)):
            somme = somme + correct_tab[j]
            nb_map = nb_map + 1
